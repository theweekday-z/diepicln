    ...
    with(processingInstance) {
        ...
        socket.on('get players', function(data){
            ...
            for(var i=0; i<data.length; i++){
                stuffs.push({x: data[i].x, y: data[i].y, name: data[i].name, score: data[i].score, lvl: data[i].lvl, r: data[i].r, d: data[i].d, id: data[i].id, playing: data[i].playing});
            }
            players=stuffs;
            var num=0;
